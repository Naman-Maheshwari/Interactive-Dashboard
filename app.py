...
select = st.sidebar.selectbox('Visualization type', ['Bar plot', 'Pie chart'], key='select_1')
sentiment_count = data['airline_sentiment'].value_counts()
sentiment_count = pd.DataFrame({'Sentiment': sentiment_count.index, 'Tweets': sentiment_count.values})

st.markdown("### Number of tweets by sentiment for all airlines")
if select == 'Bar plot':
...
each_airline = st.sidebar.selectbox('Visualization type', ['Bar plot', 'Pie chart'], key='select_2')
...
choice = st.sidebar.multiselect('Pick airlines', ('US Airways', 'United', 'American', 'Southwest', 'Delta', 'Virgin America'), key='multiselect_1')
if len(choice) > 0:
    st.subheader("Breakdown airline by sentiment")
    breakdown_type = st.sidebar.selectbox('Visualization type', ['Pie chart', 'Bar plot'], key='select_3')
...
st.sidebar.subheader("Sentiment distribution for each airline")
selected_airline = st.sidebar.selectbox('Select airline', ('All', 'US Airways', 'United', 'American', 'Southwest', 'Delta', 'Virgin America'), key='select_airline')
if selected_airline:
    st.subheader(f"Sentiment distribution for {selected_airline}")
    if selected_airline == 'All':
        sentiment_dist = data.groupby('airline_sentiment').size().reset_index(name='Tweets')
        sentiment_dist.columns = ['Sentiment', 'Tweets']
    else:
        sentiment_dist = plot_sentiment(selected_airline)
    fig_4 = px.bar(sentiment_dist, x='Sentiment', y='Tweets', color='Sentiment', title=f'Sentiment Distribution for {selected_airline}', height=500)
